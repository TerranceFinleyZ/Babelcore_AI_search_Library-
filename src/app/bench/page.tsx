...
export default function BenchPage() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden text-slate-100 font-[var(--font-geist-sans)] relative">
      ...
      {githubOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setGithubOpen(false)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-8 flex flex-col items-center gap-5 shadow-2xl max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-zinc-800 border border-zinc-700 text-white">
              <GithubIcon size={24} />
            </div>
            <div className="text-center">
              <h2 className="text-base font-semibold text-zinc-100 mb-1">Babelcore AI Search Library</h2>
              <p className="text-xs text-zinc-500">TerranceFinleyZ / Babelcore_AI_search_Library-</p>
            </div>
            <p className="text-sm font-bold text-orange-400 text-center">⭐ Don&apos;t forget to Star and Subscribe!!!</p>
            <div className="flex gap-3 w-full">
              <button
                onClick={() => setGithubOpen(false)}
                className="flex-1 py-2 rounded-lg border border-zinc-700 text-sm text-zinc-300 hover:bg-zinc-800 transition-all"
              >
                Close
              </button>
              <a
                href="https://github.com/TerranceFinleyZ/Babelcore_AI_search_Library-"
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setGithubOpen(false)}
                className="flex-1 py-2 rounded-lg bg-zinc-100 text-sm text-zinc-900 font-semibold hover:bg-white transition-all text-center flex items-center justify-center gap-2"
              >
                <GithubIcon size={15} />
                View on GitHub
              </a>
